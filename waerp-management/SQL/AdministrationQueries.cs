...
namespace waerp_management.sql
{
    internal class AdministrationQueries
    {
        ...
        public static bool AddNewUser(string user_ident, string username, string password, string name, string surname, string email, string roleStr, string[] userPrivilegesArr)
        {
            ...
            RunSqlExec($"INSERT INTO users (user_id, user_ident, username, user_password, name, surname, email, role_id) VALUES ({MaxIDStr}, '{user_ident}','{username}', '{password}', '{name}', '{surname}', '{email}', {roleID})");
            return true;
        }
        ...
        {
            return RunSql($"SELECT * FROM filter{index}_names");
        }
        ...
        public static bool DeleteLocation()
        {
            if (int.Parse(RunSql($"SELECT * FROM location_objects WHERE location_id = {CurrentLocationAdministrationModel.SelectedLocationId}").Tables[0].Rows[0]["location_quantity"].ToString()) == 0)
            {
                ...
                ErrorHandlerModel.ErrorText = "Der Lagerort wurde erfolgreich aus der Datenbank entfert!";
                ErrorHandlerModel.ErrorType = "SUCCESS";
                ErrorWindow openSuccess = new ErrorWindow();
                openSuccess.ShowDialog();
                return true;
            ...
            {
                ErrorHandlerModel.ErrorText = "Sie können keinen Lagerort löschen in dem noch etwas lagert! Bitte lagern Sie die enthaltenen Artikel erst um!";
                ErrorHandlerModel.ErrorType = "NOTALLOWED";
                ErrorWindow openNotallowed = new ErrorWindow();
                openNotallowed.ShowDialog();
                return false;
        ...
        public static bool EditLocation()
        {
            ...
            if (ds.Tables[0].Rows.Count > 0)
            {
                ErrorHandlerModel.ErrorText = $"Es existiert bereits ein anderer Lagerort mit dem Namen {CurrentLocationAdministrationModel.LocationName}!";
                ErrorHandlerModel.ErrorType = "NOTALLOWED";
        ...
        public static DataSet RunSql(string query)
        {
            ...
            {
                ErrorHandlerModel.ErrorText = ex + "\n" + query;
