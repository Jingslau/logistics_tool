...
namespace waerp_management.License
{
    internal class LicenseServerConnector
    {
            ...
            if (ds.Tables[0].Rows.Count == 1)
            {
                if (ds.Tables[0].Rows[0]["isActivated"].ToString() == "False")
                {
                    try
                    {
                        ...
                        cmd.ExecuteNonQuery();
                        RegistryKey key = Registry.CurrentUser.OpenSubKey(@"SOFTWARE\wærp-toolpilot", true);
        ...
        public static bool CheckLicense()
        {
            ...
            try
            {
                ...
                if (ds.Tables[0].Rows.Count > 0)
                {
                    ...
                    if (licenseIsValid)
                    {
                        cmd = new MySqlCommand($"UPDATE licensekeys SET last_checkin_date = '{DateTime.Now.ToString("yyyy-MM-dd HH-mm-ss")}' WHERE product = 'wærp-toolpilot' AND component_hash = '{SystemHardwareReader.GetSystemHash()}'", conn);
                        cmd.ExecuteNonQuery();
                        RegistryKey key = Registry.CurrentUser.OpenSubKey(@"SOFTWARE\wærp-toolpilot", true);
        ...
        {
            if (Registry.CurrentUser.OpenSubKey(@"SOFTWARE\wærp-toolpilot") != null)
            {
                RegistryKey key = Registry.CurrentUser.OpenSubKey(@"SOFTWARE\wærp-toolpilot", true);
