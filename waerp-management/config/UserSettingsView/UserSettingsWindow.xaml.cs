...
using waerp_management.config.UserSettingsView;
...
namespace waerp_management.config
{
    ...
    public partial class UserSettingsWindow : Window
    {
        ...
        {
            InitializeComponent();
            CurrentVersion.Text = "wærp.stockpilot Version " + Assembly.GetExecutingAssembly().GetName().Version.ToString();
        ...
        private void LoadSettings()
        {
            ...
            DatabasePort.Text = key.GetValue("DBPort").ToString();
        ...
        private void SaveSettings(object sender, RoutedEventArgs e)
        {
            ...
            AdministrationQueries.RunSqlExec($"UPDATE company_settings SET settings_value = '{GlobImagePath.Text.Replace("\\", "\\\\")}' WHERE settings_name = 'global_image_path'");
        ...
        {
            SqlConn.SetConnectionString(ServerAdress.Text, DatabaseUser.Text, DatabasePassword.Password, ServerSchema.Text, DatabasePort.Text);
            RegistryKey key = Registry.CurrentUser.OpenSubKey(@"SOFTWARE\wærp-stockpilot", true);
            ...
            key.SetValue("DBPort", DatabasePort.Text);
            key.SetValue("LetterPaperPath", PDFFilePath.Text);
            key.SetValue("HistoryLogsPath", DocumentHistory.Text);
            key.SetValue("OrderOverviewPath", DocumentOrder.Text);
        ...
        {
            SettingsStoreTmp.SmtpUrl = smtpHost.Text;
            SettingsStoreTmp.SmtpMail = companyMail.Text;
            SettingsStoreTmp.SmtpPort = int.Parse(smtpPort.Text);
            SettingsStoreTmp.SmtpPassword = companyMailPassword.Password;
            ...
            if (companyMailPassword.Password != "")
            {
                ...
                string emailAddress = SettingsStoreTmp.SmtpMail;
                string password = SettingsStoreTmp.SmtpPassword;
                string host = SettingsStoreTmp.SmtpUrl;
                int port = SettingsStoreTmp.SmtpPort;
                ...
                {
                    SettingsStoreTmp.isValidConn = false;
                }

                w.Stop();
                if (SettingsStoreTmp.isValidConn)
                {
                    ErrorHandlerModel.ErrorText = "Die Verbindung zum SMTP Server war erfolgreich!";
                    ErrorHandlerModel.ErrorType = "SUCCESS";
                    ErrorWindow showSuccess = new ErrorWindow();
                    showSuccess.ShowDialog();
                    SettingsStoreTmp.isValidConn = true;
                }
                else
                {
                    ErrorHandlerModel.ErrorText = "Die Verbindung zum SMTP Server ist fehlgeschlagen!";
                ...
                SaveSMTPConn.IsEnabled = SettingsStoreTmp.isValidConn;
            }
            else
            {
        ...
        private void ImagePathSelection_Click(object sender, RoutedEventArgs e)
        {
            using (var dialog = new FolderBrowserDialog())
            {
                DialogResult result = dialog.ShowDialog();
                ...
                if (result == System.Windows.Forms.DialogResult.OK && !string.IsNullOrWhiteSpace(dialog.SelectedPath))
                {
                    GlobImagePath.Text = dialog.SelectedPath;
                }
            }
        }
    }
