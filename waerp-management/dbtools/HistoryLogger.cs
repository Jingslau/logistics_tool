...
namespace waerp_management.dbtools
{
    internal class HistoryLogger
    {
        ...
        public static void CreateRentHistory(string item_id, string item_quantity, string item_location_old)
        {
            ...
            MySqlCommand cmd = new MySqlCommand($"INSERT INTO history_log VALUES (history_id = {maxID}, item_id = {item_id}, item_quantity={item_quantity}, item_location_old = {item_location_old}, item_location_new=0, action_id = 1, user_id = {MainWindowViewModel.UserID}, createdAt = '{sqlFormattedDate}', updatedAt = '{sqlFormattedDate}', show_trigger = 1)", conn);
            cmd.ExecuteNonQuery();
            cmd = new MySqlCommand($"UPDATE history_log SET item_id = {item_id}, item_quantity={item_quantity}, item_location_old = {item_location_old}, item_location_new=0, action_id = 1, user_id = {MainWindowViewModel.UserID}, createdAt = '{sqlFormattedDate}', updatedAt = '{sqlFormattedDate}', show_trigger = 1 WHERE history_id = {maxID}", conn);
            cmd.ExecuteNonQuery();
            conn.Close();
